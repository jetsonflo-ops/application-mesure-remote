...
class LoginView(QWidget):
    ...
    def _show_force_password_change_dialog(self, username: str):
        ...
        pw_layout = QVBoxLayout(dialog)

        # Titre informatif
        info_label = QLabel(
            f"Utilisateur {username} - Première connexion\n\n"
            "Le mot de passe par défaut a été détecté.\n"
            "Veuillez le modifier pour sécuriser votre compte."
        )
        ...
        pw_layout.addWidget(buttons)

        # Navigation clavier
        self.new_password_input.returnPressed.connect(self.confirm_password_input.setFocus)
        self.confirm_password_input.returnPressed.connect(dialog.accept)
    ...
    def handle_login(self):
        ...
        # VERIFIER LE RATE LIMITING AVANT L'AUTHENTIFICATION
        auth = self.auth_manager
        can_proceed, remaining_seconds = auth.check_rate_limit(username)

        if not can_proceed:
            # Afficher message de blocage ou cooldown
            if username == "operateur" and auth.is_operator_blocked():
                QMessageBox.warning(
                    self,
                    "Compte bloque",
                ...
                self._show_operator_block_ui()
            elif username == "Superviseur" and auth.is_supervisor_blocked():
                QMessageBox.warning(
                    self,
                    "Compte bloque",
                    "Compte Superviseur verrouille.\n"
                ...
                QMessageBox.warning(
                    ...
                    f"Veuillez reessayer dans {remaining_seconds} secondes."
                )
            # Effacer le mot de passe et retourner
